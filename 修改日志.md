...
9. 当前覆盖目标是千问双拼 `qw_double`；千问全拼 `qw` 必须保持厂商原样。
...
把 `/Users/mac/Library/Rime` 中的“星猫键道 / `xmjd6`”部署结果覆盖到千问输入法的双拼方案，同时兼容双拼入口 `schema_id: qw_double` 和千问对单字节标点的外层直提逻辑；千问全拼 `qw` 保持不变。

### 用户可见结果

- 用户选择“千问双拼”时仍加载方案 ID `qw_double`，实际使用星猫键道 `xmjd6` 的 schema、词库、Lua、OpenCC 和编译产物。
- 用户选择“千问拼音”时继续使用厂商原版 `qw` 全拼。
...
6. 把以下 `xmjd6.extended` 产物同时映射为千问双拼固定名称：
   - `xmjd6.extended.prism.bin` → `qw_double.prism.bin`
   - `xmjd6.extended.reverse.bin` → `qw_double.reverse.bin`
   - `xmjd6.extended.table.bin` → `qw_double.table.bin`
7. 从源和编译后的 `xmjd6.schema.yaml` 各生成一份 `qw_double.schema.yaml`：
   - `schema_id: xmjd6` 改成 `schema_id: qw_double`
   - `dictionary: xmjd6.extended` 改成 `dictionary: qw_double`
   - 在 `engine/processors` 首项插入且只插入一次 `lua_processor@*xmjd6/qime_trigger_compat`
   - 不修改厂商 `qw.schema.yaml`、`qw.*.bin` 或全拼词库。
8. 向千问用户目录复制：
   - `xmjd6.cx.dict.yaml`
...
grep -F 'schema_id: qw_double' "$QW/schemas/qw_double.schema.yaml"
grep -F 'dictionary: qw_double' "$QW/schemas/qw_double.schema.yaml"
test "$(grep -Fc 'lua_processor@*xmjd6/qime_trigger_compat' \
  "$QW/schemas/qw_double.schema.yaml")" -eq 1
grep -F 'name: "千问拼音"' "$QW/schemas/qw.schema.yaml"
test -f "$QW/rime_user/build/qw_double.prism.bin"
test -f "$QW/rime_user/build/qw_double.reverse.bin"
test -f "$QW/rime_user/build/qw_double.table.bin"
```

### 手工功能检查

1. 切换到千问输入法并在设置中选择“千问双拼”。
2. 确认双拼入口显示星猫键道候选和 Lua 功能；切回“千问拼音”确认厂商全拼仍可用。
3. 验证 `=`、`;`、反引号、反斜杠触发行为。
...
| 相对路径 | 权限 | 字节数 | SHA-256 |
|---|---:|---:|---|
| `scripts/sync_qianwen_rime.sh` | `0755` | 29403 | `92fa93dae343c88d34a737c3a8e707d62c8fbb94259cb72a278346847d93f85d` |
...
| `tests/test_sync_qianwen_rime.sh` | `0755` | 12617 | `50b2ed63dddffdf0baf190bd220e319cba7123ee8d180f0f0e527f077436e515` |
...
> 不要用本文记录的 `user.yaml` 时间戳覆盖新环境；不要把旧 `build/*.bin` 或旧 App 内 `qw_double.*.bin` 当成固定附件恢复，应从当前词库重新部署生成。
