...
# ── 常见姓氏 ──────────────────────────────────────────────────────────────────
SURNAMES = set('李王张刘陈杨黄赵吴周徐孙马朱胡郭何高林罗郑梁谢宋唐许韩冯邓曹彭曾肖田董袁潘于蒋蔡余杜叶程苏魏吕丁任沈姚卢姜崔钟谭陆汪范金石廖贾夏韦付方白邹孟熊秦邱江尹薛闫段雷侯龙史陶黎贺顾毛郝龚邵万钱严覃武戴莫孔向汤')
...
# ── 地名后缀 ──────────────────────────────────────────────────────────────────
# 强地名后缀：结尾是这些字，高概率是地名
GEO_STRONG_SUFFIX = set('乡镇寨苑巷坊屯堡郡坪县洲岛港')

# 弱地名后缀：需要额外判断
GEO_WEAK_SUFFIX = set('路街湾山湖河庄')
...
def is_person_name(word):
    ...
    # 高置信度人名后缀
    if suffix in NAME_BIGRAMS:
        return True

    # 名字部分两个字都是常见人名用字
    if word[1] in NAME_CHARS and word[2] in NAME_CHARS:
        return True
