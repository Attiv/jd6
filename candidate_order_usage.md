...
输入 `=tp` 会在 Rime 候选区列出 `candidate_order.txt` 中的有效记录：
...
如果该记录曾把被挤词补到新码，并继续产生链式挪码，管理面板会一并清理这些依赖记录。直接点击管理候选仍会被多数 Rime 前端作为普通文字提交，因此撤销操作统一使用“双按 `0`”。即使 switcher 当前为“调频关”，`=tp` 管理入口仍然可用。
